...
#else
	/*Pure kernel case */
	op->filp = klib_fopen(dev, O_RDWR, 0);
...
#else
	/*Pure kernel case */
	return klib_fread(buff, (int)sz, op->filp);
#endif
}
...
#else
	/*Pure kernel case */
	return klib_fwrite(buff, (int)sz, op->filp);
#endif
}
...
int64_t fop_emmc_seek(struct flash_op *op, uint64_t offs, int whence)
{
	...
	op->filp->f_pos = offs;
	return offs;
	//return klib_fseek(op->filp, (loff_t)offs, whence);
#endif
}
