...
extern unsigned int crc32 (unsigned int crc, unsigned char  *p, unsigned int len);
...
int32_t fw_ctx_to_info(struct fw_ctx *ctx, void **info)
{
	...
	*info = (void *)malloc(info_sz);
	fw_debug("%s: Allocate info buff @%p\n", __func__, *info);
...
static int32_t load_info_from_flash(void *info1, void *info2)
{
	...
	op = (struct flash_op *)flash_op_open(dev, main_flash);
...
struct fw_ctx *fw_ctx_init_from_flash(void)
{
	...
	info = get_active_info(info1, info2);
	if (!info) {
		goto failed;
	}
...
int32_t fw_ctx_check_conflict(struct fw_ctx *ctx1, struct fw_ctx *ctx2)
{
	...
	p_list1 = fw_get_part_list(ctx1);
	p_list2 = fw_get_part_list(ctx2);
