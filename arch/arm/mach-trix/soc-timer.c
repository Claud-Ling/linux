...
static int sigma_trix_timer_set_next_event(unsigned long cycles,
...
static void sigma_trix_timer_set_mode(enum clock_event_mode mode,
				    struct clock_event_device *evt)
{
	u32 period;
...
static struct clock_event_device soctimer_clockevent = {
	...
	.shift		= 32,
	.set_next_event	= sigma_trix_timer_set_next_event,
	.set_mode	= sigma_trix_timer_set_mode,
};
...
static irqreturn_t sigma_trix_timer_interrupt(int irq, void *dev_id)
{
...
static struct irqaction sigma_trix_timer_irq = {
	.name		= "soc timer1",
	.flags		= IRQF_TIMER | IRQF_IRQPOLL,
	.handler	= sigma_trix_timer_interrupt,
};

static void __init sigma_trix_clockevent_init(void)
{
	int cpu = smp_processor_id();
	...
	setup_irq(TRIHIDTV_TIMER_1_INTERRUPT, &sigma_trix_timer_irq);
	...
	soctimer_clockevent.cpumask		= cpumask_of(cpu);
...
static void __init sigma_trix_clocksource_init(void)
{
	TRI_DBG("[%d] %s\n",__LINE__,__func__);	
...
void __init trix_timer_init(void)
{
	...
	sigma_trix_clocksource_init();
	sigma_trix_clockevent_init();

#if LINUX_VERSION_CODE <= KERNEL_VERSION(3,10,0)
	setup_sched_clock(trix_clock_read_cycle, 32, SOC_TIMER_FREQ);
#else
	sched_clock_register((u64 (*)(void))trix_clock_read_cycle, 32, SOC_TIMER_FREQ);
#endif
