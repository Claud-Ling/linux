...
static int __init trix_timer_of_parse(struct device_node *np)
{
	struct clk *clk;
	int ret = 0;
	u32 clk_rate;

	trix_timer->base = of_iomap(np, 0);
	if (IS_ERR(trix_timer->base)) {
	 ...
	 */
	if (of_property_read_u32(np, "clock-source", &trix_timer->source_id)) {
		trix_timer->source_id = 0;
	}

	if (of_property_read_u32(np, "clock-event", &trix_timer->event_id)) {
		trix_timer->event_id = 1;
	}

	/* sanity check on the assignment */
	WARN_ON(trix_timer->source_id >= MAX_TIMER_NUM);
	WARN_ON(trix_timer->event_id >= MAX_TIMER_NUM);
	WARN_ON(trix_timer->source_id == trix_timer->event_id);


	/* get irq of the clock event device */
	trix_timer->ced.irq = irq_of_parse_and_map(np, trix_timer->event_id);
	if (trix_timer->ced.irq <= 0) {
	...
	trix_timer->freq = clk_rate;

out:
	return ret;
}

static void __init trix_timer_of_init(struct device_node *np)
{
	int ret;
