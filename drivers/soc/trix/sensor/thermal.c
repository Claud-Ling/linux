...
static int sigma_thermal_get_temp(void)
{
	...
	for (i=0; i<max_sensors; i++) {
		...
		sprintf(tmp, "%s %s\n", sist_get_sensor_name(i), temperature_array[min(value, (int)ARRAY_SIZE(temperature_array))]);
		strcat(buff, tmp);
		memset(tmp, 0, 64);
		idx_sum += value;
	}
	...
	avg_temp = idx_sum / i;
	sprintf(tmp, "%s %s\n", "Average", temperature_array[min(avg_temp, (int)ARRAY_SIZE(temperature_array))]);
	strcat(buff, tmp);
