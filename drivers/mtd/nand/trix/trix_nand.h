#ifndef __DRIVERS_MTD_TRIX_NAND_H__
#define __DRIVERS_MTD_TRIX_NAND_H__
#include <linux/mtd/mtd.h>    
#include <linux/mtd/nand.h>  
#include <asm/io.h> 

#define DRIVER_NAME "trix-nand"
#define NAND_DBG(f, x...) \
...
#define nand_writel(info, off, val)     \
        __raw_writel((val), (info)->host_base + (off))

#define nand_readl(info, off)           \
        __raw_readl((info)->host_base + (off))


#define NAND_BOOTROM_R_TIMG  (0x00000000) /* boot rom read timing control register */
#define NAND_FCR             (0x00000004) /* flash rom control register */
#define NAND_DCR             (0x00000008) /* data control register */
#define NAND_TCR             (0x0000000c) /* timing control register */
#define NAND_SYN_MODE0       (0x00000010) /* sync mode control 0 register */
#define NAND_SYN_MODE1	     (0x00000014) /* sync mode control 1 register */
#define NAND_SPI_MODE_CMD    (0x00000018) /* sync mode control 2 register */
#define NAND_OP_MODES        (0x0000001c) /* */
...
#define NAND_SPI_DLY_CTRL    (0x0000020) /* SPI mode clock delay control register */
#define NAND_SPI_TIMG_CTRL0  (0x0000024) /* SPI mode timing control register 0 */
#define NAND_SPI_TIMG_CTRL1  (0x0000028) /* SPI mode timing control register 1 */
#define NAND_BASE_ADDR0      (0x000002c) /* base address register 0 */
#define NAND_BASE_ADDR1      (0x0000030) /* base address register 1 */
#define NAND_BURST_TIMG      (0x0000034) /* flashrom burst timing control register */
#define NAND_BASE_ADDR2      (0x0000038) /* base address register 2 */
#define NAND_BASE_ADDR3      (0x000003c) /* base address register 3 */
#define NAND_BASE_ADDR4      (0x0000040) /* base address register 4 */

#define NAND_ONE_DATA_ECC0   (0x00000044) /* onenand flash data spcae ecc registe 0 */
#define NAND_ONE_DATA_ECC1   (0x00000048) /* onenand flash data spcae ecc registe 1 */
#define NAND_ONE_DATA_ECC2   (0x0000004c) /* onenand flash data spcae ecc registe 2 */
#define NAND_ONE_SPR_ECC0    (0x00000050) /* onenand flash spare space ecc register 0 */
#define NAND_ONE_SPR_ECC1    (0x00000054) /* onenand flash spare space ecc register 1 */
...
#define NAND_MLC_ECC_CTRL            (0x00000070) /* MLC ECC control register */
#define NAND_MLC_ESS_STATUS          (0x00000074) /* MLC ECC status register */
#define NAND_MLC_ECC_DATA            (0x00000078) /* MLC ECC data register */
...
/* ---------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------*/
#define NAND_ECC_CTRL                (0x00000200) /* ECC control register */


#define MLC_ECC_REG_CONTROL     (0x00000070) /* MLC ECC control register */
#define MLC_ECC_REG_STATUS      (0x00000074) /* MLC ECC status register */ 
...
#define REG_BCH_ECC_BASE                (0x00000200) /* ECC control register */ 
...
#define TRIX_ECC_BUSY_WAIT_TIMEOUT (1 * HZ) 

#define TRIX_DMA_TRANS_TIMEOUT     (2 * HZ) 
/* ---------------------------------------------------------------------
...
enum nand_io {
        TRIX_NAND_PIO = 0,         /* PIO mode*/
        TRIX_NAND_DMA,         /* enabled FDMA mode */
};
...
struct trix_nand_platform_data {
        unsigned int         cs;          //reserv for future.
        void __iomem         *host_base;  //nand host base addres
        void __iomem         *io_base;    //nand io space address
        enum nand_io         xfer_type;                   
};
