#define pr_fmt(fmt) "trix:nand-hcd: " fmt
...
#if defined(CONFIG_MTD_PARTS_TRIX_LEGACY)
...
static void enable_nce(struct trix_nand_info *info ,int enabled)
{
	unsigned int val;
...
static void trix_nand_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	struct trix_nand_info *info = container_of(mtd,
					struct trix_nand_info, mtd);
...
static int trix_nand_ready(struct mtd_info *mtd)
{
	struct trix_nand_info *info = container_of(mtd,
					struct trix_nand_info, mtd);
 ...
 */
void set_nand_mode(struct trix_nand_info *info,int mode)
{
	if( likely(TRIX_NAND_NEW == info->nand_ctrler) )
		MWriteRegWord((void *)(info->host_base + NAND_CCR), mode<<24, 0xFF000000);
	else if( TRIX_NAND_OLD == info->nand_ctrler)
		MWriteRegWord((void *)info->host_base + NAND_DCR, mode<<24, 0xFF000000);
}

/* Allocate the DMA buffers */ 
static int trix_nand_dma_init(struct trix_nand_info *info)
{
	if( TRIX_NAND_PIO == info->xfer_type )
		return 0;
	...
	if( ( NULL ==  info->data_buff_virt ) || ( NULL ==  info->dma_int_descr_virt ) ) {
		pr_err("dma alloc fail, free resource\n");
		goto out_free_mem ;
	}
...
out_free_mem:
	...
	/*Force to PIO mode*/
	info->xfer_type = TRIX_NAND_PIO;

	return -ENOMEM;
}


void hidtv_nand_inithw(struct trix_nand_info *info)
{
	unsigned char val8;
	unsigned int val32;
...
#ifdef CONFIG_MTD_PARTS_TRIX_LEGACY
...
static inline int parse_mtd_cmdline(struct mtd_info *mtd)
{
	...
	if(mtd_count > 0){
		pr_notice("Using partition definition from KERNEL command line parameters\n");
...
static int hidtv_nand_add_partition(struct trix_nand_info *info)
{	
	int nr_parts = 0;
...
/*PIO Mode*/
static void trix_pio_read_buf32(struct mtd_info *mtd, uint8_t *buf, int len)
{
        int i;
...
static void trix_write_buf(struct mtd_info *mtd, const uint8_t *buf, int len)
{
        struct nand_chip *chip = mtd->priv;
 ...
 */
static int trix_nand_dma_op(struct mtd_info *mtd, void *buf, int len,
                               int is_read)
{
	struct trix_nand_info *info = container_of(mtd,
					struct trix_nand_info, mtd);
	...
	unsigned long dma_timeout = jiffies + TRIX_DMA_TRANS_TIMEOUT;
	...
	WriteRegHWord((void *)0xfb000006, val16 |0x1);
        ...
        {
                pr_warn("warning:%s, DMA not ready.prev:%08x. current: %08x, xfer_type force to PIO mode\n",__func__,prev,cur);
		/* 
...
static void trix_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	struct trix_nand_info *info = container_of(mtd,
					struct trix_nand_info, mtd);
	...
	if( (TRIX_NAND_DMA == info->xfer_type) && (len > mtd->oobsize) )
	{
		if(trix_nand_dma_op(mtd, buf, len, 1) == 0)
			goto xfer_complete;
		else
			goto xfer_err;
	}
	trix_pio_read_buf32(mtd, buf ,len);	
...
xfer_err:
	 ...
	 * xfer_type will be force to TRIX_NAND_PIO from now.
	 */ 
	set_nand_mode(info, NAND_SW_MODE);
	nand_writel(info, NAND_CTRL, 0x00000420);
	trix_pio_read_buf32(mtd, buf ,len);
...
static int trix_verify_buf(struct mtd_info *mtd, const uint8_t *buf, int len)                                                     
{
        int i;                
        struct nand_chip *chip = mtd->priv;
        u32 *p = (u32 *) buf; 
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
static int trix_read_page_bch(struct mtd_info *mtd, struct nand_chip *chip,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
static void trix_write_page_bch(struct mtd_info *mtd, struct nand_chip *chip,
							const uint8_t *buf)
#else
static int trix_write_page_bch(struct mtd_info *mtd, struct nand_chip *chip,const uint8_t *buf, int oob_required)
...
static int trix_ecc_calculate(struct mtd_info *mtd, const uint8_t *data,
                                uint8_t *ecc_code)             
{
	int stat;
	struct nand_chip *chip = mtd->priv;
...
static int trix_ecc_correct(struct mtd_info *mtd, uint8_t *data,
...
static void trix_ecc_hwctl(struct mtd_info *mtd, int param)
{
	struct nand_chip *chip = mtd->priv;
...
static void trix_set_nand_ctrler(struct trix_nand_info *info)
{
#ifdef CONFIG_TRIX_HAS_NEW_NAND
	info->nand_ctrler = otp_get_new_nand_sel_state()?TRIX_NAND_NEW:TRIX_NAND_OLD;
#else
	info->nand_ctrler = TRIX_NAND_OLD;
#endif	
	pr_info("%s NAND controller selected\n",
		(info->nand_ctrler==TRIX_NAND_OLD)?"legacy":"new");
	return;
}

static int trix_nand_probe(struct platform_device *pdev)
{
	struct trix_nand_info           *info;
	struct trix_nand_platform_data  *pdata;
	int err = 0;	
	...
	info = kzalloc(sizeof(struct trix_nand_info), GFP_KERNEL); 
	if(!info) {
		err =  -ENOMEM;
		goto exit;
	}
	...
	info->nand.cmd_ctrl    = trix_nand_hwcontrol;	
	info->nand.dev_ready   = trix_nand_ready;
	
	info->nand.write_buf   = trix_write_buf;
	info->nand.read_buf    = trix_read_buf;
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
	info->nand.verify_buf  = trix_verify_buf;
#endif
	...
	switch (info->mtd.writesize)
	{
		...
		case 8192:
			...
			break;
		default:
			pr_err("ERROR: unsupport NAND device.\n");
			return -ENXIO;
	}
	...
	if (!info->nand.ecc.read_page)
		info->nand.ecc.read_page = trix_read_page_bch;
	if (!info->nand.ecc.write_page)
		info->nand.ecc.write_page = trix_write_page_bch;
	
	info->nand.ecc.calculate = trix_ecc_calculate;
	info->nand.ecc.hwctl     = trix_ecc_hwctl;
	info->nand.ecc.correct   = trix_ecc_correct;
	...
	pr_info("Detected NAND, %lldMiB, erasesize %dKiB, pagesize %dB,oobsize %dB, \
	oobavail %dB\n",
	info->mtd.size >> 20, 
	...
	pr_info("Nand ECC info:Strength=%d,ecc size=%d ecc_bytes=%d\n",
...
static int trix_nand_remove(struct platform_device *pdev)
{
	struct trix_nand_info *info = platform_get_drvdata(pdev);
...
#ifdef CONFIG_PM
static int trix_nand_suspend(struct platform_device *pdev, pm_message_t state)
{
	return 0;
}

static int trix_nand_resume(struct platform_device *pdev)
{
	return 0;
}
#else
#define trix_nand_suspend     NULL
#define trix_nand_resume      NULL
#endif

static struct platform_driver trix_nand_driver = {
        .driver = {
                .name   = "trix-nand",       
        },
        .probe          = trix_nand_probe,
        .remove         = trix_nand_remove,
        .suspend        = trix_nand_suspend,         
        .resume         = trix_nand_resume,
};

module_platform_driver(trix_nand_driver);
