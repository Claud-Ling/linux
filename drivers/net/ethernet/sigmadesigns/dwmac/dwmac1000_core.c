...
{
	void __iomem *ioaddr = hw->pcsr;
	dwmac_set_mac_addr(ioaddr, addr, GMAC_ADDR_HIGH(reg_n),
			    GMAC_ADDR_LOW(reg_n));
}
...
{
	void __iomem *ioaddr = hw->pcsr;
	dwmac_get_mac_addr(ioaddr, addr, GMAC_ADDR_HIGH(reg_n),
			    GMAC_ADDR_LOW(reg_n));
}
				 ...
				 int mcbitslog2)
{
	...
	case 8:
		...
		break;
	default:
		pr_debug("DWMAC: err in setting mulitcast filter\n");
		return;
		break;
	}
				 ...
				 struct net_device *dev)
{
	...
	if (netdev_uc_count(dev) > perfect_addr_number)
		...
		netdev_for_each_uc_addr(ha, dev) {
			dwmac_set_mac_addr(ioaddr, ha->addr,
					    GMAC_ADDR_HIGH(reg),
					    GMAC_ADDR_LOW(reg));
			reg++;
		}
...
static const struct dwmac_ops dwmac1000_ops = {
