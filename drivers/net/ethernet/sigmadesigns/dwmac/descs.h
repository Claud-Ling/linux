...
/*
 * workaround for memory coherence issue UONEQA-4
 */
static void inline desc_coherence_check(volatile unsigned int *desc, const unsigned int val)
{
	/*
	 * The memory barrriers are added in between preparation of dma descriptors
	 * to ensure all instructions before enabling the dma are complete
	 */
	wmb();
	while(le32_to_cpu(*desc) != val)
		nop();
}
